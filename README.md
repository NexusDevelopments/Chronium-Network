...
<img src="https://socialify.git.ci/NexusDevelopments/Chronium-Network/image?description=1&font=Inter&forks=1&issues=1&language=1&name=1&owner=1&pattern=Circuit%20Board&pulls=1&stargazers=1&theme=Dark" alt="ruby" width="640" height="320" />

<img alt="repo size" src="https://img.shields.io/github/repo-size/NexusDevelopments/Chronium-Network?style=for-the-badge"></img>
<img alt="website status" src="https://img.shields.io/website?url=https%3A%2F%2Fchronium.network&style=for-the-badge"></img>
<img alt="commit a week" src="https://img.shields.io/github/commit-activity/w/NexusDevelopments/Chronium-Network?style=for-the-badge"></img>

</div>

<div align="center">
    <h2>Get Started</h2>
    <a>To get started, press one of the buttons below to deploy Chronium</a>
    <br />
    <br />
    <a href="#terminal">
...
-   This will **NOT** deploy on GitHub Pages, Netlify, Gitlab Pages, or any other _static_ host
-   Vercel deployment is supported via the Astro serverless adapter
-   This will **NOT** work on Render
---

## How to get links

[![Chronium Network Discord](https://invidget.switchblade.xyz/unblocker?theme=darl)](https://discord.gg/unblocker)
...
-   You can E-Mail us at:
    -   Legal concerns: legal@chronium.network
    -   Contact: contact@chronium.network
---
...
- Themes allow you to customize Chronium's *look*.
...
>  ```
>
> A good example of using a custom font is the built-in `retro` theme [here](./database_assets/com.chronium.retro)

2. Add your colors and test! (Either with a self-hosted version of Chronium OR via a live preview (no clue when this will happen)
...
1. Clone the repo:
```bash
git clone https://github.com/NexusDevelopments/Chronium-Network --recursive && cd Chronium-Network
```
...
```bash
docker build chronium:latest
```
5. Run the docker images:

    - Prebuilt:
    ```bash
    docker run -v ./config.toml:/app/config.toml ghcr.io/NexusDevelopments/Chronium-Network:latest
    ```
    - Image you built yourself:
    ```bash
    docker run -v ./config.toml:/app/config.toml chronium:latest
    ```
...
1. Clone the repo (skip if using a prebuilt image):
```bash
git clone https://github.com/NexusDevelopments/Chronium-Network --recursive
```
...
There are a couple of configuration options for Chronium. The defaults are fine most of the time, but there are instances where you may not want certain options enabled or certain things running.
