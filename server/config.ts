...
const configFallbackPath = fileURLToPath(new URL("../config.example.toml", import.meta.url));
const selectedConfigPath = existsSync(configPath) ? configPath : configFallbackPath;

if (!existsSync(configPath)) {
    console.warn(chalk.yellow.bold('config.toml not found, using config.example.toml for defaults.'));
}

let doc = readFileSync(selectedConfigPath).toString();
